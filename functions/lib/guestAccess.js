 ...
 */
async function verifyGuestToken(groupId, token) {
    try {
 ...
 */
async function getGroupBirthdays(groupId, token, ip) {
    // Step 1: Verify token
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to fetch birthdays');
    }
}
/**
 * Add a new birthday via guest access (with contribution limit check)
 */
async function addBirthdayAsGuest(groupId, token, birthdayData, ip) {
    ...
    if (birthdayData.honeyPot && birthdayData.honeyPot.trim() !== '') {
        ...
        await recordFailedAttempt(ip);
        throw new functions.https.HttpsError('permission-denied', 'Bot detected. Access denied.');
    }
    ...
    if (currentCount >= contributionLimit) {
        ...
        throw new functions.https.HttpsError('resource-exhausted', `This guest link has reached its maximum limit of ${contributionLimit} birthday entries. Please contact the group admin.`);
    }
 ...
 */
exports.guestAccessOps = functions.https.onCall(async (data, context) => {
    const { mode } = data;
    ...
    if (mode === 'add_birthday') {
        ...
        const { groupId, token, birthdayData } = data;
...
//# sourceMappingURL=guestAccess.js.map